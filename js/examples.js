...
$(document).ready(function() 
{
	...
	$('.show_filtered').on("click", function(event) {
	...
	$('.show_none').on("click", function(event) {
	    displayNone(event);
	});
...
function vote(e) {
	...
	if ($(obj).attr("id") == "like") {
		...
		$(text).text("Liked");
		$(parent).css("background-color", "#97bf0d");
	} else if ($(obj).attr("id") == "dislike") {
		$(img).attr("src",disliked);
		$(img).css("margin-top", "2px");
		$(text).text("Disliked");
		$(parent).css("background-color", "#cb2025");
	}
}

function filterByCategory(e) {
	var obj = $(e.currentTarget)[0];
	$(obj).find("span").css("display", "initial");
	...
	var options = $(obj).parent().siblings();
	$.each(options, function(i, option) {
		var childOption = $(option).find("a");
		if ($(childOption).attr("id") != $(obj).attr("id"))  {
			$(childOption).find("span").css("display", "none");
		}
	});
	$("#box_none").css("display","none");
}
...
function displayNone(e) {
	...
	$("#box_none").css("display","initial");
}

function maximizeBox(e) {
